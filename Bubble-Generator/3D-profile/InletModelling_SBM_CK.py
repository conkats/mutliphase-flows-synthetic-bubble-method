...
verbose=1
n_max=1000#should not reach that value when running the code
z=D
casePath = '.' #str(sys.argv[1])
inletName = str('Inlet')#Name of the inlet boundary to be modelled
rhog = float(1.2)##Density of the gas
rhol = float(998.2) #Density of the liquid
#U=float(0.174)#Spina's thesis-Velocity of the mixture, superficial velocity sum to be introduced in domain not the gap velocity 0.589
U=float(0.1596)
...
#timeStepSize = float(0.001)#Time step size to be used but does not need to match starccm+ as it will be interpolated
timeStepSize=float(0.01)
tunit = 1
#tunit=float(0.01) #should be x10 >timeStepSize #Unit time scale - parameter for inlet modelling 
...
#mg_tunit = float(A*alpha*rhog*Ugs)*tunit#should be A*rhog*Ugs or alpha*U
...
#print('mgb_min',mgb_min ,'mgb_max',mgb_max)
#mgb_max=0.2
tol_mg=float(1e-5)#Tolerenace on the amount of gas to be introduced  in domain over a time 'tunit' 
#tol_mg = float(mg_tunit*0.1)
...
y=np.linspace(-4.0800e-01, -3.8200e-01,ny)
...
xv, zv, yv = np.meshgrid(x, z, y, indexing='ij')
...
coordList=np.array([np.arange(len(xvc)),xvc,yvc,zvc,np.ones(len(xvc))*dx*dz]).T#np.ones(len(xvc))*dx*dz=face area
#print (coordList.shape,xvc.shape,yvc.shape,zvc.shape)
#coordList=np.array([np.arange(len(xvc)),xvc,yvc,zvc,np.ones(len(xvc))*dx*dz*dy]).T#np.ones(len(xvc))*dx*dz*dy=volume
...
def bubbleShape(C_ID, C_t, timeInterval, shapeID, mgb, mgb_min, mgb_max, mg_StillRequired):
    ...
    if shapeID == 0:
        ...
        mg_checked = 0.0
        mg_bubbleWall = 0.0
        #coordinate center of the bubble
        ...
        #j_max = int(math.ceil((timeLoc + rg/U)/timeStepSize))+ 1#ceiling rounds to max int
        ###
        #Bug fixed-CK
        ###
        j_min = int((timeLoc - rg/U)/timeStepSize)+1
        j_max = int(math.ceil((timeLoc + rg/U)/timeStepSize))
        #print(j_min,j_max,timeLoc,rg/U,timeStepSize)
        for i in i_list:
            for j in range(j_min, j_max):
                #Check to ensure index does not exceed the length of timeVal
                #time_index = t * int(tunit / timeStepSize) + j
                #if time_index >= len(timeVal):
                #    continue
                #print(t,tunit,j,int(tunit / timeStepSize),t * int(tunit / timeStepSize) + j)
                coordPoint = np.array(
...
#                        ####*****************
#                        print("Sanity-4", VOFwVal[i, timeInterval * int(
#                            tunit / timeStepSize) + j, 0], 0.0)
#                        return False, 0.0
    ...
    if not(intersectBoundary):
        if mg_bubbleWall < (mgb-np.average(coordList[:, 4])*U*timeStepSize):
            print('too close to other bubble')
             ####*****************
            print("Sanity-5", mg_bubbleWall, mgb-np.average(coordList[:, 4])*U*timeStepSize)
            return False, 0.0
#     if TrialBoolean:
...
for t in np.arange(nIntervals):
    ...
    while (mg_tunit-mg_defined) > (tol_mg):#mg_defined increases until the target-mg_tunit
        #print(mg_tunit,'mg_defined',mg_defined, f"{mg_tunit-mg_defined:e}", tol_mg, iter )
        ...
        #mg_bubble =  0.13*(mg_tunit - mg_defined)*(0.4+random.random()*0.6
        print(mg_bubble)
...
#alphaW=VOFwVal[:,-8,0]
alphaW=VOFwVal[:,-10,0]
alphaW = alphaW.reshape(zv.shape)
#2D
#cntr=ax.contourf(xv,zv,alphaW,cmap="RdBu_r")

# 3D-when y is defined

# Collapse y-dimension by taking mean over axis=2 (y-axis) to get a 2D array for contourf
#alphaW_2d = np.mean(alphaW, axis=2)
#cntr = ax.contourf(xv[:, :, 0], zv[:, :, 0], alphaW_2d, cmap="RdBu_r")

# Use the original meshgrid for plotting
#alpha has been reshape to zv but cols have cord, time, 1
#xv[i,j,k]=x-coord
#print(alphaW.shape,xv.shape,zv.shape)

# Take first y-slice
#cntr = ax.contourf(xv[:, :, 0], zv[:, :, 0], alphaW[:, :, 0], cmap="RdBu_r")
# Or take last y-slice 
#cntr = ax.contourf(xv[:, :, 0], zv[:, :, 0], alphaW[:, :, -1], cmap="RdBu_r")
# Or take middle y-slice
cntr = ax.contourf(xv[:, :, 0], zv[:, :, 0], alphaW[:, :, ny//2], cmap="RdBu_r")
...
#plt.show()
plt.savefig('inletDefinition-VOFw-20s-3d.png', dpi=100, bbox_inches='tight')
...
print("Saving inlet profile to CSV-files. ")
file = "bubbles_time_001_vf_ck_Umix_0p174_"+str(int(endTime))+"s-test-3d.csv"
#f = open(file, 'w')
